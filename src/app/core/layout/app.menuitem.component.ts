...
@Component({
    ...
    template: `
		<ng-container>
            <div *ngIf="root && item.visible !== false" class="layout-menuitem-root-text" (click)="itemClick($event)">
    ...
    itemClick(event: Event) {
        ...
        if (this.item.items) {
            ...
            if (this.isSlim || this.isHorizontal || this.isSlimPlus) {
             ...
             this.tabService.tabItemObservable.next({code: this.item.code, icon: this.item.icon, label: this.item.label, canClose: true, routerLink: this.item.link});
        }
